...
- **電車時刻表表示**: 2026-07-20にトップ記事のヒーロー表示を廃止し、代わりに電車時刻表ウィジェットを表示するようにした(廃止に伴い、それまでヒーロー扱いだった記事は通常通り自分のカテゴリのグリッドに表示されるだけになった。記事が消えるわけではない)。直近3本を表示。番線は表記が不確実(印西牧の原は例外があり駅ごとに1つの番線に確定できない)なため表示していない。実際に「直近の電車」を判定するのは埋め込みデータではなくブラウザ側JS(閲覧者のPC/スマホがページを開いた時点のJST時刻、1秒ごとに再計算して残り時間をカウントダウン表示)で、平日/土曜・休日の判定は曜日に加えて埋め込んだ祝日リストも見る(祝日は土曜・休日ダイヤ扱い)。当駅始発の電車には●マークを付ける。種別欄は「印西牧の原駅に実際に停車するか」で色分けする(停車=通常色、通過=オレンジ)。2026-07-30に「種別名に「特」が含まれるか」という文字列ヒューリスティックから、`INZAI_MAKINOHARA_STOP_TYPES = {"普通", "特急"}`という実データに基づく判定へ変更した(印西牧の原駅自体のd=1/d=2両方向の発車データを実地確認したところ、この駅から発車する種別は「普通」「特急」のみで、「快速」「快特」「通勤特急」「アクセス特急」「エアポート快特」等はいずれもこの駅を通過することが判明したため。旧ロジックだと「アクセス特急」も「特」を含むため誤って停車扱いになっていた)。
  - **循環切り替え(4グループ)**: 2026-07-21に上り/下り2方向の切り替えボタンを追加し、2026-07-30に「品川・新橋」「東銀座・東日本橋」の2グループを追加して4グループの循環切り替えに拡張した。グループ定義は`TRAIN_GROUPS`(順序付きリスト)で管理し、ボタンを押すたびに次の要素へ進み、末尾の次は先頭に戻る: 「印西牧の原・千葉ニュータウン中央」(上り)→「浅草橋・新鎌ヶ谷」(下り)→「品川・新橋」(下り)→「東銀座・東日本橋」(下り)→(先頭に戻る)。ボタン文言は次に切り替わる先を示す(各グループが持つ`own_toggle_label`を、1つ前のグループのtoggleLabelとして使う)。例: 「上り印西牧の原、千葉ニュータウン発切り替え」「下り浅草橋、新鎌ヶ谷発切り替え」「下り品川、新橋発切り替え」「下り東銀座、東日本橋発切り替え」。
    - 初期表示の方向はJST時刻依存: 16:00〜23:59は「浅草橋・新鎌ヶ谷」(下り)、それ以外(0:00〜15:59)は「印西牧の原・千葉ニュータウン中央」(上り)がデフォルト(帰宅時間帯に自動で下りが出るように)。新規追加した「品川・新橋」「東銀座・東日本橋」はデフォルトには使わず、手動で循環させたときのみ表示される。ページ読み込み時に`defaultGroup()`が判定する。
  ...
  - **キャッシュ**: ダイヤ改正は年に数回程度しかないため、`build`のたびに問い合わせるのは過剰と判断し、`load_or_fetch_train_data()`が`train_timetable_cache.json`(gitignore対象、ローカルのみ)に取得結果を保存する。キャッシュが`TRAIN_CACHE_MAX_AGE_DAYS`(30日)以内かつ全駅分(2026-07-30時点で8駅)のデータを含む場合はネットワークアクセスなしでそれを使い、古い/存在しない/駅構成が古い(旧バージョンのキャッシュ等、駅を追加/変更した直後を含む)場合のみ再取得する。再取得が全滅した場合(サイト障害等)は多少古くてもキャッシュがあればそれを使い続ける(空表示になるより古い時刻表の方がまし)。祝日データも同じキャッシュに同梱される。取得失敗時は駅ごとに空リストのまま(その駅だけ表示されない)。
